...
def test_add_user_actor_not_member(
    org_repo, user_repo, org_user_repo
):
    ...
    cmd = AssignUserToOrganizationCommand(
        organization_id=make_org(org_repo),
...
def test_add_user_success(
    ...
    membership = org_user_repo.get(membership_id)
    assert membership.role == OrganizationRole.USER
    assert membership.is_active is True


def test_remove_owner_forbidden(org_repo,user_repo,org_user_repo):
    ...
    with pytest.raises(PermissionDenied):
        service.execute(cmd)
def test_remove_user_success(org_repo,user_repo,org_user_repo):
    org_id, owner_id, admin_id, user_id = make_org_with_users(org_repo,user_repo,org_user_repo)
...
def test_admin_cannot_change_owner(org_repo, user_repo, org_user_repo):
...
def test_owner_promotes_user(org_repo,user_repo,org_user_repo):
    org_id, owner_id, user_id = make_org_owner_user(org_repo,user_repo,org_user_repo)
...
def test_deactivate_owner_forbidden(org_repo,user_repo,org_user_repo):
    org_id, owner_id = make_org_with_owner(org_repo=org_repo,user_repo=user_repo,org_user_repo=org_user_repo)
...
def test_deactivate_user_success(org_repo,user_repo,org_user_repo):
    org_id, owner_id, admin_id, user_id = make_org_with_users(org_repo=org_repo,user_repo=user_repo,org_user_repo=org_user_repo)
