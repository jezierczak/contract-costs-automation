...
class CostNodePrepareMapper:
    """
    Flattens CostNode tree into rows for Excel prepare.
    """

    @staticmethod
    def map(nodes: Iterable[CostNode]) -> list[CostNodePrepareDTO]:
        nodes_by_id = {n.id: n for n in nodes}
        children_by_parent = CostNodePrepareMapper.group_by_parent(nodes)

        rows: list[CostNodePrepareDTO] = []

        def walk(node: CostNode) -> None:
            parent_code = None
            if node.parent_id:
    ...
    def group_by_parent(
        ...
        grouped: dict[UUID | None, list[CostNode]] = defaultdict(list)
