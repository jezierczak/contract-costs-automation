...
class ApplyContractProgressService(
    ActionHandler[ApplyContractProgressCommand, None]
):
    ...
    def execute(self, command: ApplyContractProgressCommand) -> None:
    ...
    def _apply_single_update(
        ...
        *,
        ...
        if not node:
            raise ValueError(
                f"Contract node {update.contract_node_id} not found"
            )
        ...
        if not node.is_active:
            raise ValueError(
                f"Cannot set progress on inactive node ({node.code})"
            )

        if not (Decimal("0") <= update.progress <= Decimal("1")):
            raise ValueError(
                f"Invalid progress value {update.progress} for node {node.code}"
            )
        ...
        if current_progress is not None and update.progress < current_progress:
            raise ValueError(
                f"Progress cannot decrease for node {node.code}"
            )

        # 🔥 brak duplikatu tej samej daty
        if node.progress_history.get(update.progress_date) is not None:
            raise ValueError(
                f"Progress already set for node {node.code} "
                f"on {update.progress_date}"
            )
        ...
        self._node_repo.add_progress(progress_entity)
