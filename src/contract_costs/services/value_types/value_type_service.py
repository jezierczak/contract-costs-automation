...
#         if self._repository.get_by_code(organization_id=value_type.organization_id,code=value_type.code):
#             raise ValueError("Value type with this code already exists")
#
#         self._repository.add(value_type)
#
#     def rename(self,organization_id, value_type_id: UUID, new_name: str) -> None:
#         value_type = self._get(organization_id,value_type_id)
#         updated = replace(value_type, name=new_name)
#         self._repository.update(updated)
#
#     def deactivate(self,organization_id, value_type_id: UUID) -> None:
#         value_type = self._get(organization_id,value_type_id)
#         updated = replace(value_type, is_active=False)
#         self._repository.update(updated)
#
#     def _get(self,organization_id, value_type_id: UUID) -> ValueType:
#         value_type = self._repository.get(organization_id=organization_id,value_type_id=value_type_id)
