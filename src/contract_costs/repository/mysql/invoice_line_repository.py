...
class MySQLInvoiceLineRepository(InvoiceLineRepository):
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                (
                    ...
                    str(invoice_line.contract_node_id) if invoice_line.contract_node_id else None,
                    str(invoice_line.value_type_id) if invoice_line.value_type_id else None,
    ...
    def _map_row( row: dict) -> InvoiceLine:
        return InvoiceLine(
            ...
            contract_node_id=UUID(row["cost_node_id"]) if row["cost_node_id"] else None,
            value_type_id=UUID(row["cost_type_id"]) if row["cost_type_id"] else None,
