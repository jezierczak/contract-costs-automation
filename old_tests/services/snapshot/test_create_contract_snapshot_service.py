...
def test_snapshot_created(repos):
    ...
    assert snapshot[0].contract_reference == contract.id
...
def test_costs_aggregated(repos):
    ...
    assert sum(
        v.amount_value
        for v in snapshots
        if node_snapshot_repo.get(v.node_snapshot_id).contract_node_reference in leaf_node_ids
    ) == Decimal("80")
